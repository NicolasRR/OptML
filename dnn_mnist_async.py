...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--delay", type=float, default=0.0001, help="""delay in seconds"""
    )
    parser.add_argument("--batch_size", type=int, default=64, help="""Batch size""")
    parser.add_argument("--lr", type=float, default=1e-4, help="""Learning rate""")
    parser.add_argument("--momemtum", type=float, default=0.9, help="""Momentum""")
    parser.add_argument(
        "--dataset",
        type=str,
        default="mnist",
    ...
    parser.add_argument("-m", help="Model accuracy", action="store_false")
    parser.add_argument("-ns", help="Save model", action="store_true")
    parser.add_argument(
        "-pca_gen",
    ...
    if not (
        os.path.exists(
            os.path.join(
                output_folder,
                f"mnist_async_{learning_rate}_{momentum}_{batch_size}_{mode}",
            )
        )
    ):
        os.mkdir(
    ...
    output_folder = os.path.join(
        output_folder, f"mnist_async_{learning_rate}_{momentum}_{batch_size}_{mode}"
    )
