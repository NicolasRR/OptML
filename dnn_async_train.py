...
def run_parameter_server_async(
    ...
    if val:
        ...
        if split_dataset or split_labels:
            ...
            len_train_loader = np.ceil(len(train_loader[0].dataset) / len(workers) / batch_size)
        else:
    ...
    base_name = get_base_name("async", dataset_name, len(workers)+1, train_split, learning_rate, momentum, batch_size, epochs, val, alt_model=alt_model, split_dataset= split_dataset, split_labels= split_labels, split_labels_unscaled= split_labels_unscaled, delay= delay, slow_worker_1= slow_worker_1, delay_intensity= delay_intensity, delay_type= delay_type)
