...
def split_indices_evenly(indices, num_workers):
    ...
    worker_indices = [indices[i*samples_per_worker:(i+1)*samples_per_worker] for i in range(num_workers)]

    # If there are any remaining samples, distribute them among the workers
    remaining_samples = len(indices) % num_workers
    for i in range(remaining_samples):
        worker_indices[i] = torch.cat((worker_indices[i], indices[-(i+1)].unsqueeze(0)))
