...
def create_worker_trainloaders(nb_workers, dataset_name, split_dataset, digits_mode, train_split, batch_size, model_accuracy):
    ...
    if dataset_name == "mnist":
        ...
        trainloader = torch.utils.data.DataLoader(mnist_train, batch_size=len(mnist_train))
        ...
        if not split_dataset and not digits_mode: #workers sharing samples
            train_loader  = DataLoader(mnist_train, batch_size=batch_size, sampler=SubsetRandomSampler(subsample_train_indices)) 
            if model_accuracy:
            ...
            worker_indices = subsample_train_indices.chunk(nb_workers) # Split the train_indices based on the number of workers (world_size - 1)
            split_dataset_dataloaders = [DataLoader(mnist_train, batch_size=batch_size, sampler=SubsetRandomSampler(worker_idx)) for worker_idx in worker_indices]
            if model_accuracy:
                train_loader_full= DataLoader(mnist_train, batch_size=batch_size, sampler=SubsetRandomSampler(subsample_train_indices)) 
            ...
            digit_train_loaders = [DataLoader(mnist_train, batch_size=batch_size, sampler=SubsetRandomSampler(worker_idx)) for worker_idx in worker_indices]

            if model_accuracy:
                full_digits_list = []
                for sublist in worker_indices:
                    full_digits_list.extend(sublist)
                train_loader_full= DataLoader(mnist_train, batch_size=batch_size, sampler=SubsetRandomSampler(full_digits_list)) 
                return (digit_train_loaders, train_loader_full)
            else:
                return digit_train_loaders
        
        elif dataset_name == "fashion_mnist":
            fashion_mnist_train = torchvision.datasets.FashionMNIST(root='./data', train=True, download=True, transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))

            trainloader = torch.utils.data.DataLoader(fashion_mnist_train, batch_size=len(fashion_mnist_train))
            data = next(iter(trainloader))[0]
            mean = data.mean()
            std = data.std()
            ...
            cifar10_train = torchvision.datasets.CIFAR10(root='./data', train=True, download=True, transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))

            trainloader = torch.utils.data.DataLoader(cifar10_train, batch_size=len(cifar10_train))
            data = next(iter(trainloader))[0]
            mean = data.mean()
            std = data.std()
            ...
            cifar100_train = torchvision.datasets.CIFAR100(root='./data', train=True, download=True, transform=torchvision.transforms.Compose([torchvision.transforms.ToTensor()]))

            trainloader = torch.utils.data.DataLoader(cifar100_train, batch_size=len(cifar100_train))
            data = next(iter(trainloader))[0]
            mean = data.mean()
            std = data.std()
...
def run_parameter_server(workers, dataset_name, split_dataset, digits_mode, logger, learning_rate, momentum, save_model=True, train_split=DEFAULT_TRAIN_SPLIT, batch_size=None, epochs=DEFAULT_EPOCHS, worker_accuracy=False, model_accuracy=False):

    train_loaders = create_worker_trainloaders(len(workers), dataset_name, split_dataset, digits_mode, train_split, batch_size, model_accuracy) #returns a tupple
    ...
    ps_rref = rpc.RRef(ParameterServer(len(workers), logger, learning_rate, momentum, dataset_name))
    futs = []
    ...
    if not split_dataset and not digits_mode: #workers sharing samples
        ...
        for idx, worker in enumerate(workers):
            futs.append(
                rpc.rpc_async(worker, run_worker, args=(ps_rref, train_loaders, logger, epochs, worker_accuracy))
            )
        ...
        for idx, worker in enumerate(workers):
            futs.append(
                rpc.rpc_async(worker, run_worker, args=(ps_rref, train_loaders[idx], logger, epochs, worker_accuracy))
            )
...
def run(rank, dataset_name, world_size, learning_rate, momentum, log_queue, save_model, split_dataset, train_split, batch_size, epochs, worker_accuracy, model_accuracy, digits_mode):
    ...
    if rank != 0:
        ...
        run_parameter_server([f"Worker_{r}" for r in range(1, world_size)], dataset_name, split_dataset, digits_mode, logger, learning_rate, momentum, save_model, train_split, batch_size, epochs, worker_accuracy, model_accuracy)
...
if __name__=="__main__":
    ...
    parser.add_argument('--dataset', type=str, choices=['mnist', 'fashion_mnist', 'cifar10', 'cifar100'], required=True,
    ...
    with Manager() as manager:
        ...
        mp.spawn(run, args=(args.dataset, args.world_size, args.lr, args.momentum, log_queue, save_model, split_dataset, args.train_split, args.batch_size, args.epochs, worker_accuracy, model_accuracy, digits_mode), nprocs=args.world_size, join=True)
