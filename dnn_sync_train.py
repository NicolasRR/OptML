...
def create_worker_trainloaders(nb_workers, dataset_name, split_dataset, digits_mode, train_split, batch_size, model_accuracy):
    ...
    if dataset_name == "mnist":
        ...
        trainloader = torch.utils.data.DataLoader(train_data, batch_size=len(train_data))
        ...
        if not split_dataset and not digits_mode: #workers sharing samples
            ...
            split_dataset_dataloaders = [DataLoader(train_data, batch_size=batch_size, sampler=SubsetRandomSampler(worker_idx)) for worker_idx in worker_indices]
            if model_accuracy:
