...
- `run_delay_comp.sh`: for `world_size`=2,6,11 train asynchronously for different delay types (constant and gaussian) and different delay intensities (small, medium, long), delay is applied to all workers and for some cases worker 1 is slowded down additionally
